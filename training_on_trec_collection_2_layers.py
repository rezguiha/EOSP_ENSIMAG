...
import differentiable_models_2_layers
...
#     #Selecting device 1 to be used since device 1 is busy in this cas
...
#             tf.config.experimental.set_virtual_device_configuration(gpu,[tf.config.experimental.VirtualDeviceConfiguration(memory_limit=5120)])
    ...
    for fold in range(args.folds):
        ...
        if args.IR_model == 'tf_idf':
            model = differentiable_models_2_layers.diff_TF_IDF(Collection.inverted_structure.embedding_matrix, dropout_rate=args.dropout_rate)

        elif args.IR_model == 'DIR':
            model = differentiable_models_2_layers.diff_DIR(Collection.inverted_structure.embedding_matrix, dropout_rate=args.dropout_rate)

        elif args.IR_model == 'BM25':
            model = differentiable_models_2_layers.diff_BM25(Collection.inverted_structure.embedding_matrix, dropout_rate=args.dropout_rate)
        #HR added JM model
        elif args.IR_model == 'JM':
            model = differentiable_models_2_layers.diff_JM(Collection.inverted_structure.embedding_matrix, dropout_rate=args.dropout_rate)
